    ...
    if (config.author) {
      return (
        <nav className="navbar navbar-expand-lg navbar navbar-dark" style={{ background: COLORS.coloPrim }}>
          ...
          <img className="navbar-brand" alt={""} src={logo} width="200" height="80" />
          ...
          <div className="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <Link className="nav-link my-2 my-sm-0 text-light" to="/">Cerrar Sesión<span className="sr-only">(current)</span></Link>
          </div>
        </nav>
      );
    } else {
